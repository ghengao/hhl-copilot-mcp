# Phase 1: MCP Lifecycle

Based on following MCP protocol specification, this vscode extension will be focusing on the client side of implementation and use `@modelcontextprotocol/sdk` to connect to various MCP servers. The process is roughly the following

1. Read user configuration for the extension which including `stdio/sse` server configurations.
2. When extension activated, Create a MCP Client instance for each server it connects to, if connection failed report a error message via warning notification.
3. Follow the protocol to exchange capability information and register all the tools using the private tool api interface within vscode.
4. When extension deactivated, disconnect all the servers based on the following protocol.


Following are the details of the lifecycle of the MCP Server which is specified in [here](https://spec.modelcontextprotocol.io/specification/2024-11-05/basic/lifecycle/) :
